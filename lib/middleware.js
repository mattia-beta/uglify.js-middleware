    ...
    if (regex.test(filename)) {
      ...
      var compile = function () {
        ...
        if (options.generateSourceMap) {
          ...
          sourceMapAppendFix = options.compress ? "\n" + '//@ sourceMappingURL=' + filename + '.map.js' : '';

          log('map', pathToMap);
        }

        try {
          var uglified = options.compress ? uglifyjs.minify(pathToFull, options.uglifyjs) : fs.readFileSync(pathToFull, 'utf8');
          ...
          mkdirp(path.dirname(pathToMin), 511 /* 0777 */, function onDirectoryCreated (err) {
            ...
            fs.writeFile(pathToMin, uglified.code + sourceMapAppendFix, function onMinWrite (err) {
              ...
              if (options.compress && options.generateSourceMap) {
